...
const unsigned char FrameBuffer::font8x8_basic[128][8] = {
...
void FrameBuffer::draw_text(int x, int y, const std::string& text, uint32_t color) {
    for (size_t i = 0; i < text.length(); i++) {
        draw_char(x + i * 8, y, text[i], color);
    }
}

void FrameBuffer::draw_char(int x, int y, char c, uint32_t color) {
    for (int i = 0; i < 8; i++) {
        for (int j = 0; j < 8; j++) {
            if ((font8x8_basic[static_cast<unsigned char>(c)][i] & (1 << j))) {
